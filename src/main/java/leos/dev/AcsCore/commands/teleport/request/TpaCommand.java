...
public class TpaCommand implements CommandExecutor {
    ...
    public TpaCommand(AcsCore plugin, TpaManager manager) {
        this.plugin = plugin;
        this.manager = manager;
    }

    private void sendClickable(Player target, Player from, boolean here) {
        TextComponent base = new TextComponent(from.getName() + (here ? " wants you to teleport to them." : " wants to teleport to you.") + " ");
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args.length != 1) {
            p.sendMessage(ColorUtil.color(AcsCore.get().configService().prefix() + "&fUsage: /tpa <player>"));
            return true;
        }
        ...
        manager.put(new TpaManager.Request(p.getUniqueId(), target.getUniqueId(), exp, TpaManager.Type.TPA));
        p.sendMessage(ColorUtil.color(AcsCore.get().configService().prefix() + "&fRequest sent to &a" + target.getName()));
        sendClickable(target, p, false);
        return true;
    }
}